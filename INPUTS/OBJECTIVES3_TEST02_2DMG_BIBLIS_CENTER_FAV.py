...
## INITIALIZATION
case_name = "OBJECTIVES3_TEST02_2DMG_BIBLIS_CENTER_FAV"
...
if noise_pos == 2:
    ...
    if type_noise == 1: # noise source AVS
        type_noise_str = 'AVS'
        for j in range(6 * expandXS, 7 * expandXS):
            for i in range(6 * expandXS, 7 * expandXS):
                print(SIGS12[j][i])
                dTOT2[j][i] = 0.05 * TOT[1][j * I_max + i]
    elif type_noise == 2:
        type_noise_str = 'FAV_X'
        bottom_row = 8 * expandXS
        top_row = 9 * expandXS
        left_col = 12 * expandXS
        right_col = 13 * expandXS
...
#    for i in range(I_max):
#        if dTOT2[j][i] != 0:
#            print(dTOT1[j][i], dTOT2[j][i], dNUFIS1[j][i], dNUFIS2[j][i], dSIGS12[j][i])
#            print(NUFIS[0][j * I_max + (5 * expandXS) - 1])
